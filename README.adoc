...
:alpine_link: link:https://www.alpinelinux.org[Alpine]
...
Provides a {ruby_link} focused {alpine_link} Docker image with current tooling for development purposes. Great for local development and/or continuous integration builds using the most up-to-date tooling.
...
docker pull bkuhlmann/alpine-ruby:5.10.0   # <= Pull specific version.
----
...
FROM bkuhlmann/alpine-ruby:5.10.0   # <= Specific version.
----
